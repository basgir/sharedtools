...
#         template = env.get_template("templates.html")
        with open('sharedtools/sharedtools/templates.html') as file_:
        ...
        html_out = template.render(perf_chart=perf_chart,drawdown_chart=drawdown_chart,ratio_chart = ratio_chart,monthly_table=monthly_table,
...
#         outfile = os.path.join(outputdir, 'report.html')  
    ...
    def plot_ratio_chart(self):
        trace_ratio = go.Scatter(
                x=self.ratio_curve.index.tolist(),
                y=self.ratio_curve.values.tolist(),
                name='Ratio (Strategy/Benchmark)',
                yaxis='y2',
                line = dict(color = ('rgb(22, 96, 0)')))
        layout = go.Layout(autosize=True,legend=dict(orientation="h"),
                         title='Ratio Chart',
                         yaxis=dict(title='Ratio (Strategy/Benchmark)'))
        ratio_chart = plotly.offline.plot({"data": [trace_ratio],"layout": layout}, include_plotlyjs=False,output_type='div')
    ...
    def split_kpi_table(self,kpi_table_full):
        kpi_table_full.columns = ["Strategy","Benchmark"]
        kpi_table_1 = kpi_table_full.iloc[3:16].astype(float).round(2).to_html(classes="table table-hover table-bordered table-striped",header=True)
        kpi_table_2 = kpi_table_full.iloc[16:24].astype(float).round(2).to_html(classes="table table-hover table-bordered table-striped",header=True)
        kpi_table_3 = kpi_table_full.iloc[24:32].astype(float).round(2).to_html(classes="table table-hover table-bordered table-striped",header=True)
        kpi_table_4 = kpi_table_full.iloc[32:40].astype(float).round(2).to_html(classes="table table-hover table-bordered table-striped",header=True)
        kpi_table_5 = kpi_table_full.iloc[40:].astype(float).round(2).to_html(classes="table table-hover table-bordered table-striped",header=True)
        return kpi_table_1,kpi_table_2,kpi_table_3,kpi_table_4,kpi_table_5

    def plot_daily_histogram(self):
        eq_without_0 = self.equity_curve.pct_change().values
        eq_without_0 = eq_without_0[eq_without_0 != 0 ]
        trace0 = go.Histogram(x=eq_without_0,
                              name="Strategy",
        ...
        b_without_0 = self.benchmark_curve.pct_change().values
        b_without_0 = b_without_0[b_without_0 != 0 ]                      
        trace1 = go.Histogram(x=b_without_0,
                              name="Benchmark",
                              histnorm='probability',
                              opacity=0.75,
                              marker=dict(
    ...
    def plot_daily_box(self):
        eq_without_0 = self.equity_curve.pct_change().values
        eq_without_0 = eq_without_0[eq_without_0 != 0 ]
        trace0 = go.Box(y=eq_without_0 ,
                        name="Strategy",
                        marker=dict(
                                     color=('rgb(22, 96, 167)')))
        b_without_0 = self.benchmark_curve.pct_change().values
        b_without_0 = b_without_0[b_without_0 != 0 ] 
        trace1 = go.Box(y=b_without_0,
                        name="Benchmark",
                        marker=dict(
                              color=('rgb(22, 96, 0)')))
